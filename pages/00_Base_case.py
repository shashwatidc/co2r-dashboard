...
def default_single_run(product_name,
    ...
    __, df_capex_totals_default, __, __, __, __,\
                ...
                __, df_potentials_default, __, __, __, __, __ = cached_single_run(product_name=product_name,
        product_rate_kg_day=product_rate_kg_day,
        df_products=df_products,
        FE_CO2R_0=FE_CO2R_0,
        FE_product_specified=FE_product_specified,
...
@st.cache_data(ttl = "1h")
def _svg_write(fig, center=True):
    """
...
st.write('''This is a techno-economic assessment (TEA) of low-temperature CO₂ reduction. Generate the [capital](#capital-cost) and [operating cost](#operating-cost) for a CO₂ electrolysis process converting captured CO₂
         into CO or ethylene in a low-temperature membrane electrode assembly (MEA) at neutral pH. You can also view 
         the costs of [non-aqueous CO₂R to CO](nonaqueous-CO) or [oxalic acid](nonaqueous-OA) in a flow cell, visualize the [sensitivity](MEA-CO) of costs, or assess [energy needs](#process-energy) and [emissions](#emissions). Modify the settings in 
