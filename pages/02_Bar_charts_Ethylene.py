...
#                         stack_lifetime_years = stack_lifetime_years,
#                         is_additional_capex = is_additional_capex, is_additional_opex = is_additional_opex,
#                         additional_capex_USD = additional_capex_USD, additional_opex_USD_kg = additional_opex_USD_kg,
...
if not st.session_state.is_active_error_ethylene:
    ...
    flag = {True: 1,
            False: 0}
    ### Define colors
    BM_capex_colors = [summer_r(i) for i in np.linspace(0, 1, len(df_capex_BM_vs_vbl.index)- flag[is_battery] - flag[is_additional_capex])] # battery gets its own color, so 1 less than capex length for other units
    if is_battery:
    ...
    # Opex colors
    opex_colors = [PuOr(i) for i in np.linspace(0, 0.85, len(df_opex_vs_vbl.index) - flag[is_additional_opex]) ]
    if is_additional_opex:
        opex_colors.append((0.85, 0.85, 0.85, 1)) # add in battery 

    #PuOr okay but low contrast at ends
    # Emissions colors
    emissions_colors = [RdYlBu(i) for i in np.linspace(0, 1, sum(~df_emissions_vs_vbl.T.isnull().all()) )  ] # len(df_energy_vs_vbl.index) - 2)] # last rows are totals
